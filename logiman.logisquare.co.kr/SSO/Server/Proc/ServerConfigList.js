// 그리드
var GridID = "#ServerConfigListGrid";
var GridSort = [];
...
function fnGridInit() {
    // 그리드 레이아웃 생성
    fnCreateGridLayout(GridID, "KeyName");
    ...
    AUIGrid.setRendererProp(GridID, AUIGrid.getColumnIndexByDataField(GridID, "BtnDelServerConfig"),
        {
            onclick: function (rowIndex, columnIndex, value, item) {
                fnDelServerConfig(item);
            }
        });
}
...
function fnCreateGridLayout(strGID, strRowIdField) {
    ...
    AUIGrid.bind(GridID, "columnStateChange", function (event) {
        fnSaveColumnLayoutAuto(GridID, "ServerConfigListGrid");
        return;
    });
};
        ...
        },
        {
            ...
            renderer: {
                ...
                onClick: function (event) {
                    fnInsServerConfig("설정값 수정", event.item.ServerType, event.item.KeyName);
                }
            }
        },
        {
            dataField: "BtnDelServerConfig",
            headerText: "삭제",
            editable: false,
            width: 100,
            renderer: {
...
// 데이터 삭제
var DelRowItem;
function fnDelServerConfig(item) {

    var strConfMsg;
    var strCallType;

    strCallType = "ServerConfigDelete";
    strConfMsg = "설정값을 삭제하시겠습니까?";

    if (item.ServerType === "" || item.KeyName === "") {
        fnDefaultAlert("설정값 삭제에 필요한 정보가 부족합니다.", "warning");
        return;
    }
    ...
    //Confirm
    fnDefaultConfirm(strConfMsg, "fnDelServerConfigProc", strCallType);

    return;
}

function fnDelServerConfigProc(ojbParam) {
    var strHandlerURL = "/SSO/Server/Proc/ServerHandler.ashx";
    var strCallBackFunc = "fnAjaxDelServerConfig";

    let objParam = {
        CallType: ojbParam,
        ServerType: DelRowItem.ServerType,
        KeyName: DelRowItem.KeyName
    };

    UTILJS.Ajax.fnHandlerRequest(objParam, strHandlerURL, strCallBackFunc, true, "", "", true);
}

function fnAjaxDelServerConfig(data) {
    if (data[0].RetCode !== 0) {
