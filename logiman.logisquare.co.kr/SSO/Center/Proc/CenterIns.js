...
function fnSetInitData() {
    if ($("#hidMode").val() === "insert") {
        ...
        $("#CenterType option:not(:selected)").attr("disabled", true);
        $("#CenterType").addClass("read");
        $("#BtnChkCorpNo").hide();
...
function fnCenterInsReload(intCenterCode) {
    document.location.replace("/SSO/Center/CenterIns?CenterCode=" + intCenterCode);
    parent.$("#lbl_LAYER_TITLE").text("회원사 수정");
