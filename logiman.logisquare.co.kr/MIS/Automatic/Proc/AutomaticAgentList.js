...
function GetCharData() {
    ...
    if ($("#DateType").val() === "M") {
        strCallType = "StatTransRateAgentMonthlyList";
    } else {
        strCallType = "StatTransRateAgentDailyList";
    }
...
function fnSetGridDataSetting() {
    AUIGrid.setGridData("#AgentChartGrid", []);
    ...
    var ColumnLayout = [
        ...
        },
        {
            ...
            renderer: { // HTML 템플릿 렌더러 사용
                ...
                var template = "<a href=\"javascript:fnAgentDetail('" + item.ItemKey + "', '" + value + "', '" + item.CenterCode + "');\" class=\"my_a_tag\" title=\"상세페이지 이동\">";
                template += value;
                template += '</a>';
...
//차트 아이콘 클릭 시
function fnAgentDetail(item, AgentName, CenterCode) {
    fnOpenRightSubLayer("담당자 상세", "/MIS/Automatic/AutomaticAgentDetail?AgentCode=" + item.replace("_", "") + "&AgentName=" + AgentName + "&DateFrom=" + $("#DateFrom").val() + "&DateTo=" + $("#DateTo").val() + "&DateType=" + $("#DateType").val() + "&OrderItemCodes=OA007" + "&CenterCode=" + CenterCode, "1024px", "700px", "70%");
    return;
}
/*********************/
