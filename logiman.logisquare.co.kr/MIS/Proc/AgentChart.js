...
function GetCharData() {
    ...
    if ($("#DateType").val() === "M") {
        SearchYMD = $("#DateYear").val();
        strCallType = "StatOrderAgentMonthlyList";
    } else {
        SearchYMD = $("#DateYear").val() + $("#DateMonth").val();
        strCallType = "StatOrderAgentDailyList";
    }
    ...
    var ColumnLayout = [
        {
            ...
            renderer: { // HTML 템플릿 렌더러 사용
                ...
                if (value !== "합계") {
                    if (!value) return "";
                    var template = "<a href=\"javascript:fnAgentDetail('" + item.ItemKey + "');\" class=\"my_a_tag\" title=\"상세페이지 이동\">";
                    template += value;
                    template += '</a>';
...
//차트 아이콘 클릭 시
function fnAgentDetail(item) {
    var ItemCode = [];

    var AgentCode = item.replace("_", "");
    var OrderItemCodes = "";
    ...
    fnOpenRightSubLayer("담당자 상세", "/MIS/AgentChartDetail?AgentCode=" + AgentCode + "&DateYear=" + $("#DateYear").val() + "&CenterCode=" + $("#CenterCode").val() + "&OrderItemCodes=" + OrderItemCodes, "1024px", "700px", "70%");
    return;
}
/*********************/
