...
UTILJS.Ajax =
{
    ...
    fnFileHandlerRequest: function (strFormParameter, strCallUrl, strCallBack, isBlock, strFailCallBack, strCompleteCallBack, isaSync) {
...
UTILJS.Util =
{
    ...
    /* 연월일 체크 */
    fnValidYMD: function (strYMD) {
        // 'YYYYMMDD' 형식의 정규표현식
        var strRegex = /^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])$/;

        // 정규표현식에 맞지 않으면 false 반환
        if (!strRegex.test(strYMD)) {
            return false;
        }

        // 연, 월, 일을 추출
        var intYear = parseInt(strYMD.substring(0, 4), 10);
        var intMonth = parseInt(strYMD.substring(4, 6), 10);
        var intDay = parseInt(strYMD.substring(6, 8), 10);

        // 월별 최대 일수를 정의 (+윤년체크)
        var arrDaysInMonth = [31, ((intYear % 4 === 0 && intYear % 100 !== 0) || (intYear % 400 === 0) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

        // 일자가 해당 월의 최대 일수를 초과하면 false 반환
        if (intDay > arrDaysInMonth[intMonth - 1]) {
            return false;
        }
    ...
    /* 시간,분 체크 */
    fnValidHM: function (strHM) {
        // HHMM 형식의 정규표현식
        var strRegex = /^(0[0-9]|1[0-9]|2[0-3])[0-5][0-9]$/;
        return strRegex.test(strHM);
    },
