window.name = "WebDomesticListGrid";
// 그리드
var GridID = "#WebDomesticListGrid";
var GridSort = [];
...
function fnCreateGridLayout(strGID, strRowIdField) {
    ...
    AUIGrid.bind(GridID, "columnStateChange", function (event) {
        fnSaveColumnLayoutAuto(GridID, 'WebDomesticListGrid');
        return;
    });
};
...
function fnGridCellDblClick(event) {
    ...
    if (strKey === "OrderStatusM" && event.item.OrderStatusM !== "등록" && event.item.OrderStatusM !== "접수") {
        ...
        fnWebDomesticIns(objItem.OrderNo, objItem.ReqSeqNo);
    }
}
...
function fnWebDomesticIns(strOrderNo, strReqSeqNo) {
    strOrderNo = (typeof strOrderNo === "undefined" || strOrderNo === null) ? "" : strOrderNo;
    var strTitle = "내수오더 " + (strOrderNo === "" ? "등록" : "수정") + (Math.floor(Math.random() * 1000) + 1);
    var strUrl = "/WEB/Domestic/WebDomesticIns" + (strOrderNo === "" ? "" : ("?OrderNo=" + strOrderNo + "&ReqSeqNo=" + strReqSeqNo));
    window.open(strUrl, strTitle, "width=1400, height=700px, scrollbars=Yes");
    return;
}
