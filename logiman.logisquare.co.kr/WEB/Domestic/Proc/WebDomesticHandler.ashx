<%@ WebHandler Language="C#" Class="WebDomesticHandler" %>
...
///================================================================
/// <summary>
/// FileName        : WebDomesticHandler.ashx
...
/// </summary>
///================================================================
public class WebDomesticHandler : AshxBaseHandler
{
    //상수 선언
    private const string CurrentMenuLink = "/WEB/Domestic/WebDomesticList"; //필수

    // 메소드 리스트
    private const string MethodWebOrderList                 = "WebOrderList"; //웹오더 조회
    private const string MethodWebOrderExcelList            = "WebOrderExcelList"; //웹오더 엑셀 다운로드
    private const string MethodConsignorList                = "ConsignorList"; //화주 조회
    private const string MethodPlaceList                    = "PlaceList"; //상하차지 조회
    private const string MethodWebOrderInsert               = "WebOrderInsert"; //오더등록
    private const string MethodWebOrderUpdate               = "WebOrderUpdate"; //오더수정
    private const string MethodWebDomesticDispatchCarList   = "WebDomesticDispatchCarList"; //배차차량조회
    private const string MethodWebDomesticPayList           = "WebDomesticPayList"; //비용 조회
    ...
    private string strReqTransType              = string.Empty;
    ...
    public override void ProcessRequest(HttpContext context)
    {
        ...
        objMethodAuthList.Add(MethodWebDomesticDispatchCarList, MenuAuthType.ReadOnly);
        objMethodAuthList.Add(MethodWebDomesticPayList,         MenuAuthType.ReadOnly);
    ...
    private void GetData()
    {
        try
        {
            ...
            strFTLFlag                   = SiteGlobal.GetRequestForm("FTLFlag"); // 2023-03-16 by shadow54 : 자동운임 수정 건
            ...
            strGoodsNote                 = SiteGlobal.GetRequestForm("GoodsNote");
            ...
            strReqTransType              = Utils.IsNull(SiteGlobal.GetRequestForm("ReqTransType"), "1");
        ...
        try
        {
            switch (strCallType)
            {
                ...
                case MethodWebOrderUpdate:
                    SetWebOrderUpdate();
                    break;
                case MethodWebDomesticDispatchCarList:
                    GetWebDomesticDispatchCarList();
                    break;
                case MethodWebDomesticPayList:
    ...
    protected void GetWebOrderExcelList()
    {
        ...
        try
        {
            ...
            lo_dtData.Columns.Add(new DataColumn("(상)연락처",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("(상)휴대폰번호", typeof(string)));

            lo_dtData.Columns.Add(new DataColumn("상차지 주소",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("상차지 상세주소", typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("상차지 비고",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("하차요청일",    typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("하차요청시간",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("(하)하차지명",  typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("(하)담당자",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("(하)연락처",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("하차지 주소",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("하차지 상세주소", typeof(string)));
                
            lo_dtData.Columns.Add(new DataColumn("하차지 비고",   typeof(string)));
            ...
            lo_dtData.Columns.Add(new DataColumn("화물품목",     typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("화물(제품)명",  typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("수량(ea)",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("중량(kg)",   typeof(string)));
            lo_dtData.Columns.Add(new DataColumn("화물길이(cm)", typeof(string)));
            ...
            {
                lo_dtData.Rows.Add(row.OrderRegTypeM, row.OrderNo, row.OrderStatusM, row.ConsignorName,row.PickupYMD
                    ...
                    ,row.GetPlace, row.GetPlaceChargeName, row.GetPlaceChargeTelNo, row.GetPlaceAddr, row.GetPlaceAddrDtl
                    ,row.GetPlaceNote, row.CarTonCodeM, row.CarTypeCodeM, row.GoodsRunTypeM, row.GoodsItemCodeM
                    ,row.GoodsName, row.Volume, row.Weight, row.Length, row.GoodsNote
                    ,row.NoteClient, row.SaleSupplyAmt, row.RegAdminName, row.RegDate, row.AcceptAdminID
                    ,row.AcceptDate);
            }

            lo_objExcel = new SpreadSheet {SheetName = "내수오더"};
    ...
    protected void SetWebOrderInsert() 
    {
        ...
        try
        {
            lo_objWebOrderModel = new WebOrderModel
            {
                ...
                ReqChargeTel              = objSes.TelNo,
                ReqChargeCell             = objSes.MobileNo,
    ...
    /// 내수 배차 목록
    /// </summary>
    protected void GetWebDomesticDispatchCarList()
    {
