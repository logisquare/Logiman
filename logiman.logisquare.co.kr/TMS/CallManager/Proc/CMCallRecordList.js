window.name = "CMCallRecordListGrid";
// 그리드
var GridID = "#CMCallRecordListGrid";
...
function fnGridCellClick(event) {
    ...
    if (strKey == "SendNumber") {
        var arrPhones = [event.item.SendNumber];
        fnOpenConnectCallGrid(event.orgEvent, arrPhones, "", event.item.CenterCode, "", 1);
        return false;
    }

    if (strKey == "RcvNumber") {
        var arrPhones = [event.item.RcvNumber];
        fnOpenConnectCallGrid(event.orgEvent, arrPhones, "", event.item.CenterCode, "", 1);
        return false;
    }
    return false;
...
function fnCallGridData(strGID) {
    var strHandlerURL = "/TMS/CallManager/Proc/CMCallRecordHandler.ashx";
    ...
    var objParam = {
        ...
        DateTo: $("#DateTo").val(),
        LogCallType: $("#CallType").val() == "" ? "" : ($("#CallType").val() == "1" || $("#CallType").val() == "3" ? "1" : "2"),
        CallKind: $("#CallType").val() == "" ? "" : ($("#CallType").val() == "1" || $("#CallType").val() == "2" ? "call" : "sms"),
        CallNumber: $("#SearchType").val() == "1" ? $("#SearchText").val() : "",
        RcvNumber: $("#SearchType").val() == "2" ? $("#SearchText").val() : "",
        SendNumber: $("#SearchType").val() == "3" ? $("#SearchText").val() : "",
        Message: $("#SearchType").val() == "4" ? $("#SearchText").val() : "",
        PageNo: $("#PageNo").val(),
