// 그리드
var GridID = "#OrderDispatchListGrid";
var GridSort = [];
var GridFilter = null;
...
$(document).ready(function () {
    ...
    $("#DateChoice").on("change", function () {
        fnChangeDatePeriod($("#DateChoice").val(), "DateFrom", "DateTo");
    });
...
function fnCreateGridLayout(strGID, strRowIdField) {
    ...
    AUIGrid.bind(GridID, "columnStateChange", function (event) {
        fnSaveColumnLayoutAuto(GridID, 'OrderDispatchListGrid');
        return;
    });
};
...
function fnGetDefaultColumnLayout() {
    var objColumnLayout = [
        ...
        },
        {
            ...
            filter: { showIcon: true },
            viewstatus: false,
            headerStyle: "aui-grid-user-custom-dispatch1",
        },
        {
...
function fnGridCellDblClick(event) {
    ...
    if (strKey === "Hawb") {
        window.open('/TMS/Unipass/UnipassDetailList?Hawb=' + objItem.Hawb + "&PickupYMD=" + objItem.PickupYMD.substring(0, 4) + "&HidMode=Inout", '화물통관 진행정보', 'width=1200px,height=800px,scrollbars=yes');
        return false;
    }
...
function fnGridCellClick(event) {
    ...
    if (key == "BtnSms") {
        ...
        fnOpenRightSubLayer("문자 발송", "/TMS/Common/MsgSend?OrderNos=" + OrderNo + "&HidCenterCode=" + CenterCode + "&HidOrderType=2", "1024px", "700px", "50%");
        return;
    }*/
}
...
function fnDispatchTypeUpdConfirm() {
    ...
    if ($("#DispatchType").val() === "") {
        fnDefaultAlertFocus("배송구분을 선택해주세요.", "DispatchType", "warning");
        return;
    }
    ...
    if (intValidType === 3) {
        fnDefaultAlert("직송 오더가 포함되어 있습니다.", "warning");
        return false;
    }
    ...
    if (intValidType === 6) {
        fnDefaultAlert("수탁 오더는 배차구분을 변경할 수 없습니다.", "warning");
        return false;
    }
    ...
    if ($("#DispatchType").val() === "1") {
        strConfirmText = "미배차";
    } else if ($("#DispatchType").val() === "2") {
        strConfirmText = "직송";
    } else if ($("#DispatchType").val() === "3") {
        strConfirmText = "집하";
    }

    fnDefaultConfirm(strConfirmText + "로 변경하시겠습니까?", "fnDispatchTypeUpd", OrderNo);
    return;
}
...
function fnDispatchTypeUpd(OrderNo) {
    ...
    var objParam = {
        ...
        CenterCode: $("#CenterCode").val(),
        GoodsDispatchType: $("#DispatchType").val(),
...
//사업장 변경
function fnLocationCodeUpdConfirm() {
    var OrderNo = [];
    var CheckedItems = AUIGrid.getCheckedRowItems(GridID);
    var intValidType = 0;
    ...
    fnDefaultConfirm(OrderNo.length + "건을 " + $("#LocationCode option:selected").text() + "(으)로 변경하시겠습니까?", "fnLocationCodeUpd", OrderNo);
    return;
}

function fnLocationCodeUpd(OrderNo) {
    var strHandlerURL = "/TMS/Dispatch/Proc/OrderDispatchHandler.ashx";
    var strCallBackFunc = "fnGridLocationCodeUpdResult";

    var objParam = {
        CallType: "OrderLocationCodeUpd",
        CenterCode: $("#CenterCode").val(),
...
function fnGridLocationCodeUpdResult(data) {
    if (data[0].RetCode !== 0) {
        fnDefaultAlert(data[0].ErrMsg, "error");
        return;
    } else {
...
function fnCreateDispatchCarGridLayout(strGID, strRowIdField) {
    ...
    objGridProps.rowIdField = strRowIdField; // 키 필드 지정
    objGridProps.enableFilter = true; //필터 표시
    objGridProps.useContextMenu = true; //필터 표시
