// 그리드
var GridID = "#OrderDispatchCollectListGrid";
var GridSort = [];
var GridFilter = null;
...
function fnSetInitAutoComplete() {
    ...
    if ($("#SearchCarNo").length > 0) {
        fnSetAutocomplete({
            ...
            callbacks: {
                getUrl: () => {
                    return "/TMS/Dispatch/Proc/OrderDispatchCollectHandler.ashx";
                },
...
function fnCreateGridLayout(strGID, strRowIdField) {
    ...
    AUIGrid.bind(GridID, "columnStateChange", function (event) {
        fnSaveColumnLayoutAuto(GridID, 'OrderDispatchCollectListGrid');
        return;
    });
};
...
function fnGetDefaultColumnLayout() {
    var objColumnLayout = [
        ...
        {
            dataField: "JCargopassOrderNo",
            headerText: "JCargopassOrderNo",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "JCargopassFlag",
            headerText: "JCargopassFlag",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "JContractStatus",
            headerText: "JContractStatus",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "JContractType",
            headerText: "JContractType",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "JNetworkNo",
            headerText: "JNetworkNo",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "GCargopassOrderNo",
            headerText: "GCargopassOrderNo",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "GCargopassFlag",
            headerText: "GCargopassFlag",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "GContractStatus",
            headerText: "GContractStatus",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "GContractType",
            headerText: "GContractType",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        ...
        {
            dataField: "GNetworkNo",
            headerText: "GNetworkNo",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
...
function fnGridCellDblClick(event) {
    ...
    if ((strKey === "JCargopassFlag" || strKey === "JCargopassInfo") && objItem.JCargopassFlag === "Y") {
        var strCenterCode = (typeof objItem.CenterCode === "undefined" || objItem.CenterCode === null) ? "" : objItem.CenterCode;
        var strCargopassOrderNo = (typeof objItem.JCargopassOrderNo === "undefined" || objItem.JCargopassOrderNo === null) ? "" : objItem.JCargopassOrderNo;
    ...
    if ((strKey === "GCargopassFlag" || strKey === "GCargopassInfo") && objItem.GCargopassFlag === "Y") {
        var strCenterCode = (typeof objItem.CenterCode === "undefined" || objItem.CenterCode === null) ? "" : objItem.CenterCode;
        var strCargopassOrderNo = (typeof objItem.GCargopassOrderNo === "undefined" || objItem.GCargopassOrderNo === null) ? "" : objItem.GCargopassOrderNo;
        var strURL = "/TMS/Cargopass/CargopassIns?CenterCode=" + strCenterCode + "&CargopassOrderNo=" + strCargopassOrderNo;
        $("#IfrmCargopass").attr("src", strURL);
        $("#DivCargopassIns").show();
        return false;
...
function fnOrderDispatchInsConfirm() {
    ...
    if (!$('input:radio[name$=DispatchType]').is(':checked')) {
        fnDefaultAlert("집하 또는 간선을 선택해주세요", "warning");
        return;
    }
    ...
    $.each(CheckedItems, function (index, item) {
        ...
        if (item.item.DispatchSeqNo3 !== "0" && item.item.DispatchSeqNo2 === "0" && item.item.GContractType === 3 ) {
            if ($('input:radio[name$=DispatchType]:checked').val() == "2") {
                intValidType = 2;
                return false;
            }
        }

        if (item.item.DispatchSeqNo2 !== "0" && item.item.DispatchSeqNo3 === "0" && item.item.JContractType === 3) {
            if ($('input:radio[name$=DispatchType]:checked').val() == "3") {
                intValidType = 3;
                return false;
            }
        }

        if (item.item.JContractType === 2) {
            if ($('input:radio[name$=DispatchType]:checked').val() == "2") {
                intValidType = 4;
                return false;
            }
        ...
        if ($("input[name$='DispatchType']:checked").val() === "2") {
            if (item.item.JCargopassFlag === "Y") {
                intValidType = 7;
                return false;
            }

        } else if ($("input[name$='DispatchType']:checked").val() === "3") {
            if (item.item.GCargopassFlag === "Y") {
                intValidType = 7;
                return false;
            }
        }
    ...
    if (intValidType === 2) {
        fnDefaultAlert("간선 오더(수탁)가 포함되어있습니다.", "warning");
        return false;
    }

    if (intValidType === 3) {
        fnDefaultAlert("집하 오더(수탁)가 포함되어있습니다.", "warning");
        return false;
    }

    if (intValidType === 4) {
        fnDefaultAlert("집하 오더(위탁)는 배차 할 수 없습니다.", "warning");
        return false;
    }

    if (intValidType === 5) {
        fnDefaultAlert("간선 오더(위탁)는 배차 할 수 없습니다.", "warning");
        return false;
    }
    ...
    if (intValidType === 6) {
        fnDefaultAlert("검색된 차량의 회원사와 오더의 회원사가 서로 다릅니다.<br>배차 차량을 다시 검색해주세요.", "warning");
        return false;
    }
...
function fnOrderDispatchIns(strOrderNos) {
    ...
    var objParam = {
        ...
        DispatchType: $("input[name$='DispatchType']:checked").val(), //집하/간선
...
function fnOrderDispatchCnlConfirm() {
    ...
    if (CheckedItems.length <= 0) {
        fnDefaultAlert("선택 된 오더가 없습니다.");
        return;
    }

    if (!$('input:radio[name$=DispatchType]').is(':checked')) {
        fnDefaultAlertFocus("취소 할 집하 또는 간선을 체크해주세요.", "DispatchType2", "warning");
        return;
    }
    ...
    $.each(CheckedItems, function (index, item) {
        ...
        if ($("input[name$='DispatchType']:checked").val() === "2") {
            if (item.item.DispatchSeqNo2 != "0") {
                DispatchSeqNos.push(item.item.DispatchSeqNo2);
            }
        } else {
            if (item.item.DispatchSeqNo3 != "0") {
                DispatchSeqNos.push(item.item.DispatchSeqNo3);
            }
        }
    });
    ...
    if (DispatchSeqNos.length <= 0) {
        fnDefaultAlert("해당 차량을 다시 확인해주세요.", "warning");
        return false;
    }
    ...
    fnDefaultConfirm("[오더" + CheckedItems.length + "건]배차를 취소하시겠습니까?", "fnOrderDispatchCnl", DispatchSeqNos);
    return;
}
...
function fnDirectUpdConfirm() {
    ...
    $.each(CheckedItems, function (index, item) {
        ...
        if (item.item.GContractType === 3 || item.item.JContractType === 3) {
            intValidType = 4;
        }

        if (item.item.GContractType === 2 || item.item.JContractType === 2) {
            intValidType = 5;
        }
    ...
    if (intValidType === 4) {
        fnDefaultAlert("직송 변경이 불가능한 오더(수탁)가 있습니다.", "warning");
        return false;
    }

    if (intValidType === 5) {
        fnDefaultAlert("직송 변경이 불가능한 오더(위탁)가 있습니다.", "warning");
        return false;
    }
    ...
    fnDefaultConfirm("직송으로 변경하시겠습니까?", "fnDispatchTypeUpd", OrderNo);
    return;
}
...
function fnDispatchCollectConfirm() {
    var OrderNo = [];
    var CheckedItems = AUIGrid.getCheckedRowItems(GridID);
    var intValidType = 0;
    ...
    if ($("#LocationCode").val() === "") {
        fnDefaultAlertFocus("사업장을 선택해주세요.", "LocationCode", "warning");
        return;
    }

    $.each(CheckedItems, function (index, item) {
        /*if (item.item.DispatchRefSeqNo2 == "0" || item.item.DispatchRefSeqNo3 == "0") {
            intValidType = 1;
            return false;
        }*/
        if (item.item.GoodsSeqNo === "0" || item.item.GoodsSeqNo === "") {
        ...
        if (item.item.JContractType === 2 || item.item.GContractType === 2 || item.item.JContractType === 3 || item.item.GContractType === 3) {
            intValidType = 4;
            return false;
        }
    ...
    if (intValidType === 1) {
        fnDefaultAlert("집하 또는 간선이 배차되지 않은 경우, 배송요청이 불가합니다.", "warning");
        return false;
    }
    ...
    if (intValidType === 4) {
        fnDefaultAlertFocus("위수탁 오더는 배송요청을 할 수 없습니다.", "CenterCode", "warning");
        return false;
    }
    ...
    fnDefaultConfirm(OrderNo.length + "건 " + $("#LocationCode option:selected").text() + "으로 배송 요청하시겠습니까?", "fnDispatchCollectIns", OrderNo);
    return;
}

function fnDispatchCollectIns(OrderNos) {
    var strHandlerURL = "/TMS/Dispatch/Proc/OrderDispatchCollectHandler.ashx";
    var strCallBackFunc = "fnGridLocationReqSuccResult";

    var objParam = {
        CallType: "OrderDeliveryLocationReqIns",
        CenterCode: $("#CenterCode").val(),
...
function fnGridLocationReqSuccResult(objData) {
    if (objData[0].RetCode !== 0) {
        fnDefaultAlert(objData[0].ErrMsg, "error");
        return;
    } else {
        fnDefaultAlert("배송요청 되었습니다.", "success");
        fnCallGridData(GridID);
        return;
    }
}
...
//집하적재, 간선적재 비용등록
//*******************************************************************************************//
var SubGridID = "#OrderDispatchCollectCostGrid";
var GridSortSub = [];
...
function fnGridInitSub() {
    // 그리드 레이아웃 생성
    fnCreateGridLayoutSub(SubGridID, "");
    
    fnSetGridEvent(SubGridID, "", "", "fnGridKeyDownSub", "", "", "", "", "fnGridCellDblClickSub");

    // 사이즈 세팅
    var intHeight = $(document).height() - 292;
    AUIGrid.resize(SubGridID, $(".grid_type_03 div.right").width(), intHeight);

    // 브라우저 리사이징
    $(window).on("resize", function () {
        AUIGrid.resize(SubGridID, $(".grid_type_03 div.right").width(), $(document).height() - 292);
    });

    //그리드에 포커스
    AUIGrid.setFocus(SubGridID);
    
    $("input[name$='DispatchTypeRadio']").on("click", function () {
        $("#CarNo").val("");
        fnCallGridDataSub(SubGridID);
    });

    // 전체 체크박스 클릭 이벤트 바인딩
    AUIGrid.bind(SubGridID, "rowAllChkClick", function (event) {
        if (event.checked) {
            // name 의 값들 얻기
            var uniqueValues = AUIGrid.getColumnDistinctValues(event.pid, "DispatchCarNo");
            // Anna 제거하기
            uniqueValues.splice(uniqueValues.indexOf(""), 1);
            AUIGrid.setCheckedRowsByValue(event.pid, "DispatchCarNo", uniqueValues);
        } else {
            AUIGrid.setCheckedRowsByValue(event.pid, "DispatchCarNo", []);
        }
    });
}
...
function fnCreateGridLayoutSub(strGID, strRowIdField) {
    ...
    objGridProps.rowCheckVisibleFunction = function(rowIndex, isChecked, item) {
        if (item.DispatchCarNo === "") {
            return false;
        }
        return true;
    }
    ...
    AUIGrid.bind(SubGridID, "columnStateChange", function (event) {
        fnSaveColumnLayoutAuto(SubGridID, 'OrderDispatchCollectCostGrid');
        return;
    });
};
...
function fnGetDefaultColumnLayoutSub() {
    var objColumnLayout = [
        ...
        //숨김
        {
            ...
            dataField: "DispatchSeqNo2",
            headerText: "집하",
            editable: false,
            width: 0,
            visible: false
        }, {
            dataField: "DispatchSeqNo3",
            headerText: "간선",
            editable: false,
            width: 0,
            visible: false
...
function fnGridCellDblClickSub(event) {
    if ($('input[name$="DispatchTypeRadio"]:checked').val() === "2") {
        $("#CarNo2").val(event.item.DispatchCarNo);
        $("#CarNo3").val("");
    } else {
        $("#CarNo2").val("");
        $("#CarNo3").val(event.item.DispatchCarNo);
    }
...
function fnCallGridDataSub(strGridID) {
    ...
    var objParam = {
        CallType: "OrderDispatchCollectPayList",
        CenterCode: $("#CenterCode").val(),
        ListType: 3,
        DateType: $("#DateType").val(),
        DateFrom: $("#DateFrom").val(),
        ...
        DispatchAdminName: $("#DispatchAdminName").val(),
        GoodsDispatchType: 3,
        DispatchType: $("input[name$='DispatchTypeRadio']:checked").val(),
        ComName: $("#ComName").val(),
        PageNo: $("#PageNo").val(),
        PageSize: $("#PageSize").val()
    };
...
function fnGridToggle() {
    if ($("div.grid_type_03 div.right").width() > 0) {
        $("div.grid_type_03 div.right").animate({
            width: 0
        }, 0);

        /*$("div.grid_type_03 div.left").animate({
            width: "98%"
        }, 200, function () {
            // 사이즈 세팅
            var intHeight = $(document).height() - 292;
            AUIGrid.resize(SubGridID, $(".grid_type_03 div.left").width(), intHeight);
        });
        $(".aui-grid").animate({
            width: "100%"
        }, 200);

        $(".aui-grid-left-main-panel").prev().animate({
            width: "100%"
        }, 200);*/
        $("button.grid_btn").addClass("close");
    } else {
        $("div.grid_type_03 div.right").animate({
            width: "27%"
        }, 200);
        ...
        $("button.grid_btn").remoceClass("close");
    }
}

//집하/간선 적재 비용등록
function fnCollectPayConfirm(flag) {
    ...
    if (flag === 1) {
        if ($("#PurchaseOrgAmt").val() === "") {
            fnDefaultAlertFocus("비용을 입력해주세요.", "PurchaseOrgAmt", "warning");
            return;
        }
    } else {
        strText = "삭제"
    ...
    fnDefaultConfirm("비용" + strText + " 하시겠습니까?", "fnCollectPay", flag);
    return;
}

function fnCollectPay(flag) {
    var LocationCode = [];
    var ItemCode = [];
    var DispatchInfos = [];
    var intValidType = 0;
    ...
    $.each(CheckedItems, function (index, item) {
        ...
        if (item.item.DispatchCarNo != "") {
            DispatchInfos.push(item.item.DispatchInfo);
        }
    });

    if (intValidType === 1) {
        fnDefaultAlert("선택 조회한 회원사와 오더의 회원사가 서로 다릅니다.", "warning");
        return false;
    }
    ...
    if (DispatchInfos.length > 200) {
        fnDefaultAlert("최대 200건까지 변경가능합니다.", "warning");
        return false;
    }

    var strHandlerURL = "/TMS/Dispatch/Proc/OrderDispatchCollectHandler.ashx";
    var strCallBackFunc = "fnGridCollectPaySuccResult";

    var objParam = {
        CallType: "OrderDispatchCollectPayIns",
        CenterCode: $("#CenterCode").val(),
        DateType: $("#DateType").val(),
        DateFrom: $("#DateFrom").val(),
        DateTo: $("#DateTo").val(),
        GoodsDispatchType: 3, //집하/간선
        DispatchType: $("input[name$='DispatchTypeRadio']:checked").val(),
        OrderLocationCodes: LocationCode.join(","),
        OrderItemCodes: ItemCode.join(","),
        SearchClientType: $("#SearchClientType").val(),
        ...
        CnlFlag: "N",
        DispatchAdminName: $("#DispatchAdminName").val(),
        PurchaseSupplyAmt: $("#PurchaseSupplyAmt").val(),
        ProcType: flag,
        ComName: $("#ComName").val(),
        DispatchInfo: DispatchInfos.join(","),
    };
    UTILJS.Ajax.fnHandlerRequest(objParam, strHandlerURL, strCallBackFunc, true, "", "", true);
}

function fnGridCollectPaySuccResult(objData) {
    if (objData[0].RetCode !== 0) {
        fnDefaultAlert(objData[0].ErrMsg, "error");
        return;
    } else {
        fnDefaultAlert("처리 되었습니다.", "success");
        fnCallGridData(GridID);
        fnCallGridDataSub(SubGridID);
        return;
    }
...
function fnContractUpd() {
    ...
    if (!$('input:radio[name$=DispatchType]').is(':checked')) {
        fnDefaultAlertFocus("집하 또는 간선을 체크해주세요.", "DispatchType2", "warning");
        return;
    }

    $.each(CheckedItems, function (index, item) {
        if ($('input:radio[name$=DispatchType]:checked').val() == "2") {
            if (item.item.DispatchRefSeqNo2 !== "0") {
                intValidType = 1;
                strTypeMsg = "집하";
            }
        } else {
            if (item.item.DispatchRefSeqNo3 !== "0") {
                intValidType = 1;
                strTypeMsg = "간선";
            }
        }
        ...
        if (item.item.JContractType === 3 || item.item.GContractType === 3) {
            intValidType = 4;
        }

        if ($('input:radio[name$=DispatchType]:checked').val() == "2") {
            if (item.item.GoodsDispatchType == 3 && item.item.CnlFlag == "N" && (item.item.DispatchRefSeqNo2 == 0) && !(item.item.JContractType == 2 && item.item.JContractStatus == 2) && item.item.JContractType != 3) {
                cnt++;
                OrderNos.push(item.item.OrderNo);
            } else {
                intValidType = 3;
            ...
            if (item.item.GoodsDispatchType == 3 && item.item.CnlFlag == "N" && (item.item.DispatchRefSeqNo3 == 0) && !(item.item.GContractType == 2 && item.item.GContractStatus == 2) && item.item.GContractType != 3) {
                cnt++;
                OrderNos.push(item.item.OrderNo);
            } else {
                intValidType = 3;
    ...
    if (cnt <= 0) {
        fnDefaultAlert("위탁할 수 있는 오더가 없습니다.<br/>(집하/간선, 취소x, 미배차, 위탁x, 위탁받음x, 위탁상태x, 이관받음x)", "warning");
        return false;
    }

    if (intValidType === 1) {
        fnDefaultAlert("이미 " + strTypeMsg +" 배차된 오더가 있습니다.<br>위탁하시려면 배차를 취소해주세요.", "warning");
        return false;
    }
    ...
    if (intValidType === 4) {
        fnDefaultAlert("수탁 오더는 위탁 할 수 없습니다.", "warning");
        return false;
    }
    ...
    fnOpenRightSubLayer("위탁 운송사 선택", "/TMS/Dispatch/OrderDispatchContract?GridID=" + GridID.replace("#", "") + "&HidCenterCode=" + $("#CenterCode").val(), "1024px", "700px", "50%");
}
...
function fnContractCnlConfirm() {
    ...
    $.each(CheckedItems, function (index, item) {
        ...
        if ($('input:radio[name$=DispatchType]:checked').val() == "2") {
            if (item.item.JContractStatus == 0 && item.item.GContractStatus == 2) {
                intValidType = 2;
            }
        } else {
            if (item.item.JContractStatus == 2 && item.item.GContractStatus == 0) {
                intValidType = 3;
            }
        }
    ...
    if (intValidType === 2) {
        fnDefaultAlert("간선 위탁이 포함된 오더가 있습니다.<br>집하 위탁 오더만 선택해주세요.", "warning");
        return false;
    }

    if (intValidType === 3) {
        fnDefaultAlert("집하 위탁이 포함된 오더가 있습니다.<br>간선 위탁 오더만 선택해주세요.", "warning");
        return false;
    }

    if (intValidType === 4) {
        fnDefaultAlert("수탁 오더는 위탁 취소 할 수 없습니다..", "warning");
        return false;
    }
...
function fnOpenDispatchCar(intCenterCode, intOrderNo, intGoodsSeqNo) {
    $("#HiddenCenterCode").val(intCenterCode);
...
function fnCallDispatchCarGridData(strGID) {
    ...
    var objParam = {
        ...
        CenterCode: $("#HiddenCenterCode").val(),
...
function fnOpenRegCargopass(intDispatchType) {
    var objArrOrderNo = [];
    var strOrderNos = "";
    var intCnt = 0;
    var intNoCnt = 0;
    ...
    $.each(CheckedItems, function (index, item) {
        if (item.item.CnlFlag !== "Y"
            ...
            && item.item.GoodsDispatchTypeM === "집하"
            && item.item.TransType != 3
            && ((intDispatchType == 2
                && !(item.item.JContractType == 2 && item.item.JContractStatus == 2)
                && (item.item.JNetworkNo == "" || item.item.JNetworkNo == "0")
                && item.item.JCargopassFlag !== "Y"
                && item.item.DispatchRefSeqNo2 <= 0)
                || (intDispatchType == 3
                   && !(item.item.GContractType == 2 && item.item.GContractStatus == 2)
                   && (item.item.GNetworkNo == "" || item.item.GNetworkNo == "0")
                   && item.item.GCargopassFlag !== "Y"
                   && item.item.DispatchRefSeqNo3 <= 0))) {
...
function fnOpenRegCargopassConfirm(objParam) {
    var strURL = "/TMS/Cargopass/CargopassIns?DispatchType=" + objParam.DispatchType + "&CenterCode=" + objParam.CenterCode + "&OrderNos=" + objParam.OrderNos;
    $("#IfrmCargopass").attr("src", strURL);
    $("#DivCargopassIns").show();
    return false;
}
...
function fnGetDispatchCountSuccResult(objRes) {
    if (objRes) {
        ...
        $("#BtnOpenRegCargopass2").text("집하카고패스등록 " + objRes[0].data.NetworkDispatchType2Cnt);

        if (objRes[0].data.NetworkDispatchType2Cnt == "0") {
            $("#BtnOpenRegCargopass2").removeClass("border_on");
        }

        $("#BtnOpenRegCargopass3").text("간선카고패스등록 " + objRes[0].data.NetworkDispatchType3Cnt);

        if (objRes[0].data.NetworkDispatchType3Cnt == "0") {
            $("#BtnOpenRegCargopass3").removeClass("border_on");
        }
...
function fnBorderSetting() {
    ...
    var strBtnOpenRegCargopass2 = $("#BtnOpenRegCargopass2").text().replace("집하카고패스등록", "").replace(/ /gi, "");

    if (!isNaN(Number(strBtnOpenRegCargopass2))) {
        if (Number(strBtnOpenRegCargopass2) > 0) {
            if ($("#BtnOpenRegCargopass2").hasClass("border_on")) {
                $("#BtnOpenRegCargopass2").removeClass("border_on");
            } else {
                $("#BtnOpenRegCargopass2").addClass("border_on");
            }
        }
    }

    var strBtnOpenRegCargopass3 = $("#BtnOpenRegCargopass3").text().replace("간선카고패스등록", "").replace(/ /gi, "");

    if (!isNaN(Number(strBtnOpenRegCargopass3))) {
        if (Number(strBtnOpenRegCargopass3) > 0) {
            if ($("#BtnOpenRegCargopass3").hasClass("border_on")) {
                $("#BtnOpenRegCargopass3").removeClass("border_on");
            } else {
                $("#BtnOpenRegCargopass3").addClass("border_on");
            }
        }
    }
