...
function fnSetInitAutoComplete() {
    ...
    if ($("#ClientName").length > 0) {
        fnSetAutocomplete({
            ...
            callbacks: {
                getUrl: () => {
                    return "/TMS/ClientPlace/Proc/ClientPlaceChargeHandler.ashx";
                },
...
function fnClientPlaceChargeInsConfirm() {
    ...
    var strCallType = "";
    if (fnInsValidCheck()) {
        strCallType = "ClientPlaceCharge" + $("#HidMode").val();
...
function fnLocationUpdateMode(PlaceSeqNo) {
    location.href = "/TMS/ClientPlace/ClientPlaceChargeIns?HidMode=Update&PlaceSeqNo=" + PlaceSeqNo;
}
...
function fnChargeInsConfirm() {
    ...
    var strConfMsg = "";
    var strCallType = "";
    strCallType = "Charge" + ($("#SeqNo").val() != "0" ? "Update" : "Insert");
    strConfMsg = "담당자를 " + ($("#SeqNo").val() != "0" ? "수정" : "등록");
    strConfMsg += "하시겠습니까?";
...
function fnCallChargeGridData(strGID) {
    ...
    var objParam = {
        CallType: "ChargeList",
        PlaceSeqNo: $("#PlaceSeqNo").val(),
        ChargeName: $("#SearchChargeName").val()
    };
...
function fnPlaceNameChk() {
    ...
    if ($("#PlaceName").val() === "") {
        fnDefaultAlertFocus("상차지명을 입력해주세요.", "PlaceName", "warning");
        return;
    }

    var objParam = {
        CallType: "ClientPlaceChargeList",
        CenterCode: $("#CenterCode").val(),
        SearchType: "PlaceNameSearch",
        SearchText: $("#PlaceName").val()
    };
