...
function fnGridInit() {
    // 그리드 레이아웃 생성
    fnCreateGridLayout(GridID, "ConsignorCode");
...
function fnCreateGridLayout(strGID, strRowIdField) {
    ...
    AUIGrid.bind(GridID, "columnStateChange", function (event) {
        fnSaveColumnLayoutAuto(GridID, 'ConsignorListGrid');
        return;
    });
};
...
function fnGridCellDblClick(event) {
    ...
    fnConsignorUpd(objItem.ConsignorCode);
}
...
function fnConsignorUpd(strCode) {
    fnOpenRightSubLayer("화주 수정", "/TMS/Client/ConsignorIns?ConsignorCode=" + strCode, "500px", "700px", "50%");
}