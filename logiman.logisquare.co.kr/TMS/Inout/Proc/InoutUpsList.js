window.name = "InoutUpsListGrid";
// 그리드
var GridID = "#InoutUpsListGrid";
var GridSort = [];
...
function fnCallGridData(strGID) {
    ...
    var strHandlerURL = "/TMS/Inout/Proc/InoutUpsHandler.ashx";
...
function fnInoutUpsExcel() {
    var CheckedItems = AUIGrid.getCheckedRowItems(GridID);
    var intValidType = 0;
    var ArrPayClientChargeName = [];
    var OrderNos = [];
    var LocationCode = [];
    var ItemCode = [];
    var OrderItemCodeM = "";
    ...
    if (CheckedItems.length === 0) {
        fnDefaultAlert("내역을 선택해주세요.", "warning");
        return false;
    }

    $.each(CheckedItems, function (index, item) {
        if (ArrPayClientChargeName.indexOf(item.item.PayClientChargeName) === - 1) {
            ArrPayClientChargeName.push(item.item.PayClientChargeName);
        }
        $.each(CheckedItems, function (id, Subitem) {
            if (item.item.OrderItemCode !== Subitem.item.OrderItemCode) {
                intValidType = 1;
            }
        });
        OrderNos.push(item.item.OrderNo);

        OrderItemCodeM = item.item.OrderItemCodeM;
    });
    
    if (intValidType === 1) {
        fnDefaultAlert("하나의 상품만 선택해주세요.");
        return;
    }
    ...
    var objParam = {
        ...
        OrderItemCodes: ItemCode.join(","),
        PayClientCode: $("#PayClientCode").val(),
        PayClientChargeNames: ArrPayClientChargeName.join(","),
        OrderItemCodeM: OrderItemCodeM
    };

    $.fileDownload("/TMS/Inout/Proc/InoutUpsHandler.ashx", {
        httpMethod: "POST",
        data: objParam,
