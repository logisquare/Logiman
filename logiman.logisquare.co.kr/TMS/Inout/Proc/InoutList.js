window.name = "InoutListGrid";
// 그리드
var GridID = "#InoutListGrid";
var GridSort = [];
var GridFilter = null;
...
function fnCostIns() {
    window.open("/TMS/Inout/InoutCostIns", "수출입 비용등록", "width=1720, height=875px, scrollbars=Yes");
    return;
}

/*
function fnWebReqIns() {
    window.open("/TMS/Inout/InoutWebReqList", "웹등록요청", "width=1324, height=700px, scrollbars=Yes");
    return;
}
function fnWebReqUpd() {
    window.open("/TMS/Inout/InoutWebReqList", "웹수정요청", "width=1324, height=700px, scrollbars=Yes");
    return;
}
*/
...
function fnChgOrderLocation() {
    ...
    fnDefaultConfirm(strConfMsg, "fnChgOrderLocationProc", "InoutLocationUpdate");
    return;
}
...
//서비스이슈현황
function fnSQIList() {
    window.open("/TMS/Common/SQIList?OrderType=2", "서비스이슈현황", "width=1350, height=700, scrollbars=Yes");
    return false;
}

//카운트 표시
function fnGetInoutCount() {
    var strHandlerURL = "/TMS/Inout/Proc/InoutHandler.ashx";
    var strCallBackFunc = "fnGetInoutCountSuccResult";
...
function fnGetInoutCountSuccResult(objRes) {
    if (objRes) {
        if (objRes[0].RetCode) {
            if (objRes[0].RetCode !== 0) {
                fnGetInoutCountCallBack();
                return false;
            }
        }
...
function fnGetInoutCountCallBack() {
    fnBorderSetting();
    setTimeout(fnGetInoutCount, 1000 * 60);
}
...
function fnMsgSend(type) {
    ...
    fnOpenRightSubLayer("문자 발송", "/TMS/Common/MsgSend?ParamOrderNos=" + OrderNos + "&ParamCenterCode=" + $("#CenterCode").val() + "&ParamOrderType=" + type, "1024px", "700px", "50%");
    return;
}
/*********************************************/
...
function fnWebOrderChg() {
    window.open("/TMS/Common/WebOrderRequestChgList?ListType=2", "웹수정요청", "width=840, height=600, scrollbars=Yes");
    return false;
}
