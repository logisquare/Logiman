// 그리드
var GridID = "#DepositMonthlyNoteListGrid";
var GridSort = [];

var ViewCalendarFlag = false; //캘린더 날짜 선택 표시 여부
$(document).ready(function () {
    $("#NoteYMD").datepicker({
        dateFormat: "yy-mm-dd"
    });
    $("#NoteYMD").datepicker("setDate", GetDateToday("-"));


    // 브라우저의 키 이벤트 세팅
    $(this).keydown(function (event) {
        // Ctrl + F
        if (event.ctrlKey && event.keyCode === 70) {
            fnSearchDialog("gridDialog", "open");
            return false;
        }

        // ESC
        if (event.keyCode === 27) {
            fnSearchDialog("gridDialog", "close");
            return false;
        }
    });

    // 그리드 초기화
    fnGridInit();

    // 그리드 검색 이벤트
    if ($("#gridDialog").length > 0) {
        $("#LinkGridSearchClose").on("click", function () {
            fnSearchDialog("gridDialog", "close");
            return false;
        });

        $("#BtnGridSearch").on("click", function () {
            fnSearchClick();
            return false;
        });

        $("#GridSearchText").on("keydown", function (event) {
            if (event.keyCode === 13) {
                fnSearchClick();
                return false;
            }

            if (event.keyCode === 27) {
                fnSearchDialog("gridDialog", "close");
                return false;
            }
        });
    }

    fnSetInitData();
});

function fnSetInitData() {
    if ($("#HidErrMsg").val()) {
        fnDefaultAlert($("#HidErrMsg").val(), "warning", "fnWindowClose()");
        return false;
    }

    $("#CenterCode option:not(:selected)").prop("disabled", true);

    fnCallClient();
}

function fnWindowClose() {
    parent.$("#cp_layer").css("left", "");
    parent.$("#cp_layer").toggle();
}

function fnGridInit() {
    // 그리드 레이아웃 생성
    fnCreateGridLayout(GridID, "NoteSeqNo");

    /* 기본 이벤트 바인딩
        Function SetGridEvent(strGID, ready, sChange, kDown, nFound, hClick, fDblClick, cClick, cDblClick)
        ex) SetGridEvent(GridID, "fnGridReady", "fnGridSelectionChange", "fnGridKeyDown", "fnGridNotFound", "fnGridHeaderClick", "fnGridFooterDblClick", "fnGridCellClick", "fnGridCellDblClick");
    */
    fnSetGridEvent(GridID, "", "", "fnGridKeyDown", "fnGridSearchNotFound", "", "", "", "fnGridCellDblClick");

    // 사이즈 세팅
    var intHeight = $(document).height() - 450;
    AUIGrid.resize(GridID, $(".grid_list").width(), intHeight);

    // 브라우저 리사이징
    $(window).on("resize", function () {
        AUIGrid.resize(GridID, $(".grid_list ").width(), $(document).height() - 450);

        clearTimeout(window.resizedEnd);
        window.resizedEnd = setTimeout(function () {
            AUIGrid.resize(GridID, $(".grid_list ").width(), $(document).height() - 450);
        }, 100);
    });

    //그리드에 포커스
    AUIGrid.setFocus(GridID);
}

//기본 레이아웃 세팅
function fnCreateGridLayout(strGID, strRowIdField) {

    var objGridProps = {};

    objGridProps.enableSorting = true; //다중 칼럼 필드 정렬(Sorting) 여부
    objGridProps.enableMovingColumn = true; //칼럼 헤더를 드래그앤드랍으로 자리를 옮기는 기능 활성화
    objGridProps.useGroupingPanel = false; // 그룹핑 패널 사용	
    objGridProps.processValidData = true; // 숫자 정렬
    objGridProps.noDataMessage = "검색된 데이터가 없습니다."; // No Data message
    objGridProps.headerHeight = 25; // 헤더 높이 지정
    objGridProps.rowHeight = 25; //로우 높이 지정
    objGridProps.selectionMode = "multipleCells"; // 셀 선택 모드
    objGridProps.copySingleCellOnRowMode = true; //셀렉션모드(selectionMode) 가 "singleRow" 또는 "multipleRows" 인 경우 복사(Ctrl+C) 할 때 단일 셀을 복사할지 여부를 지정
    objGridProps.fixedColumnCount = 0; // 고정 칼럼 개수
    objGridProps.showFooter = true; // 푸터 보이게 설정
    objGridProps.footerHeight = 25; //푸터 높이
    objGridProps.showRowNumColumn = true; // 줄번호 칼럼 렌더러 출력
    objGridProps.showRowCheckColumn = false; // 체크박스 표시 렌더러 출력
    objGridProps.rowIdField = strRowIdField; // 키 필드 지정
    objGridProps.enableFilter = true; //필터 표시
    objGridProps.editable = false; // 편집 수정 가능 여부
    objGridProps.showStateColumn = false; //좌측에 행의 상태를 나타내는 칼럼 출력 여부
    objGridProps.softRemoveRowMode = false; //수정 가능한 상태(editable = true) 인 경우 행을 삭제하면 그리드에서 바로 제거하지 않고, 삭제된 표시를 하고 남겨 둘 지 여부
    objGridProps.enableRestore = false; //편집 가능 그리드(editable=true)에서 수정, 추가, 삭제 행을 원래 상태로 복구 가능하게 할지 여부
    objGridProps.isGenNewRowsOnPaste = false; //다수의 행을 클립 보드 붙여넣기(Ctrl+V) 할 때 그리드의 마지막 하단 행보다 클립보드 양이 많은 경우, 새 행을 만들고 붙여넣기 할지 여부
    objGridProps.showTooltip = true; //툴팁 표시 여부
    objGridProps.filterMenuItemMaxCount = 200; //필터링 가능한 체크박스의 최대값
    objGridProps.applyRestPercentWidth = true; //칼럼 레이아웃 작성 시 칼럼의 width 를 퍼센티지(%) 로 설정
    objGridProps.fillColumnSizeMode = true;

    var strItemKey = strGID.replace("#", "");
    var objLoadLayout = fnLoadColumnLayout(strItemKey, "fnGetDefaultColumnLayout()");
    var objOriLayout = fnGetDefaultColumnLayout();
    var objResultLayout = fnGetLayoutOptions(objOriLayout, objLoadLayout);

    //그리드 출력
    AUIGrid.create(strGID, objResultLayout, objGridProps);

    // 푸터
    fnSetGridFooter(GridID);
    AUIGrid.setGridData(GridID, []);
};

function fnSetDetailList(objItem) {

    $("#CenterCode").val(objItem.CenterCode);
    $("#HidCenterCode").val(objItem.CenterCode);
    $("#HidComCode").val(objItem.ComCode);

    var objCheckedRow = AUIGrid.getCheckedRowItems(GridID);
    if (objCheckedRow.ComCode !== objItem.ComCode) {
        AUIGrid.setCheckedRowsByValue(GridID, "ComCode", objItem.ComCode);
    }

    if (!$("#HidCenterCode").val() || !$("#HidComCode").val()) {
        return false;
    }

    AUIGrid.setGridData(GridDepositID, []);

    fnCallDetailGridData(GridDepositID);
}

// 컬럼 세팅 - 사용자 정의(페이지 기능별 수정 필요)
function fnGetDefaultColumnLayout() {
    var objColumnLayout = [        
        {
            dataField: "NoteYMD",
            headerText: "확인일",
            editable: false,
            width: "10%",
            dataType: "date",
            formatString: "yyyy-mm-dd",
            filter: { showIcon: true },
            viewstatus: false
        },
        {
            dataField: "Note",
            headerText: "확인내용",
            editable: false,
            width: "50%",
            filter: { showIcon: true },
            style: "aui-grid-text-left",
            viewstatus: false
        },
        {
            dataField: "RegDate",
            headerText: "등록일",
            editable: false,
            width: "10%",
            filter: { showIcon: true },
            viewstatus: false
        },
        {
            dataField: "RegAdminName",
            headerText: "등록자",
            editable: false,
            width: "10%",
            filter: { showIcon: true },
            viewstatus: false
        },
        {
            dataField: "UpdDate",
            headerText: "수정일",
            editable: false,
            width: "10%",
            filter: { showIcon: true },
            viewstatus: false
        },
        {
            dataField: "UpdAdminName",
            headerText: "수정자",
            editable: false,
            width: "10%",
            filter: { showIcon: true },
            viewstatus: false
        },
        /*숨김필드*/
        {
            dataField: "ClientCode",
            headerText: "ClientCode",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        },
        {
            dataField: "ClientName",
            headerText: "ClientName",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        },
        {
            dataField: "CenterCode",
            headerText: "CenterCode",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        },
        {
            dataField: "CenterName",
            headerText: "CenterName",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        },
        {
            dataField: "NoteSeqNo",
            headerText: "NoteSeqNo",
            editable: false,
            width: 0,
            visible: false,
            viewstatus: false
        }
    ];

    return objColumnLayout;
}

//---------------------------------------------------------------------------------
//---- 이벤트 핸들러 START
//---------------------------------------------------------------------------------
function fnGridCellDblClick(event) {
    var strKey = AUIGrid.getDataFieldByColumnIndex(GridID, event.columnIndex);
    var objItem = event.item;

    if (strKey.indexOf("Btn") > -1) {
        return;
    }

    fnSetNote(objItem);
}

// 키 다운 핸들러 - 사용자 정의(페이지 기능별 수정 필요)
function fnGridKeyDown(event) {
    // Ctrl + F
    if (event.ctrlKey && event.keyCode === 70) {
        fnSearchDialog("gridDialog", "open");
        return false;
    }

    // ESC
    if (event.keyCode === 27) {
        fnSearchDialog("gridDialog", "close");
        return false;
    }
    return true;
}

// 검색 notFound 이벤트 핸들러
function fnGridSearchNotFound(event) {
    var strTerm = event.term; // 찾는 문자열
    var blWrapFound = event.wrapFound; // wrapSearch 한 경우 만족하는 term 이 그리드에 1개 있는 경우.

    if (blWrapFound) {
        fnDefaultAlert("그리드 마지막 행을 지나 다시 찾았지만 다음 문자열을 찾을 수 없습니다 - " + strTerm, "warning");
    } else {
        fnDefaultAlert("다음 문자열을 찾을 수 없습니다 - " + strTerm, "warning");
    }
    return false;
};

//---------------------------------------------------------------------------------
//---- 이벤트 핸들러 END
//---------------------------------------------------------------------------------

//그리드 데이터 호출 - 사용자 정의(페이지 기능별 수정 필요)
function fnCallGridData(strGID) {

    if (!$("#HidCenterCode").val() || !$("#HidClientCode").val()) {
        return false;
    }

    var strHandlerURL = "/TMS/ClosingSaleDeposit/Proc/DepositMonthlyHandler.ashx";
    var strCallBackFunc = "fnGridSuccResult";

    var objParam = {
        CallType: "PayMisuNoteList",
        CenterCode: $("#HidCenterCode").val(),
        ClientCode: $("#HidClientCode").val()
    };

    // ajax 요청 전 그리드에 로더 표시
    AUIGrid.showAjaxLoader(strGID);

    UTILJS.Ajax.fnHandlerRequest(objParam, strHandlerURL, strCallBackFunc, false, "", "", true);
}

// 그리드 데이터 호출 성공 시 - 사용자 정의(페이지 기능별 수정 필요)
function fnGridSuccResult(objRes) {
    if (objRes) {
        AUIGrid.setGridData(GridID, []);

        if (objRes[0].RetCode) {
            if (objRes[0].RetCode !== 0) {
                fnDefaultAlert(objRes[0].ErrMsg, "warning");
                return false;
            }
        }

        if (objRes[0].result.ErrorCode !== 0) {
            fnDefaultAlert(objRes[0].result.ErrorMsg, "warning");
            return false;
        }

        AUIGrid.setGridData(GridID, objRes[0].data.list);
        AUIGrid.removeAjaxLoader(GridID);
        return false;
    }
}

//푸터 설정 - 사용자 정의(페이지 기능별 수정 필요)
function fnSetGridFooter(strGID) {
    // 푸터 설정
    var GridFooterObject = [
        {
            positionField: "CenterName",
            dataField: "CenterName",
            operation: "COUNT",
            formatString: "#,##0",
            postfix: "건",
            style: "aui-grid-text-right"
        }
    ];

    // 푸터 객체 세팅
    AUIGrid.setFooter(strGID, GridFooterObject);
}

//업체정보 조회
function fnCallClient() {

    var strHandlerURL = "/TMS/ClosingSaleDeposit/Proc/DepositMonthlyHandler.ashx";
    var strCallBackFunc = "fnCallClientSuccResult";
    var strFailCallBackFunc = "fnCallClientFailResult";

    var objParam = {
        CallType: "ClientList",
        CenterCode: $("#HidCenterCode").val(),
        ClientCode: $("#HidClientCode").val()
    }

    UTILJS.Ajax.fnHandlerRequest(objParam, strHandlerURL, strCallBackFunc, true, strFailCallBackFunc, "", false);
}

function fnCallClientSuccResult(objRes) {

    if (objRes) {
        if (objRes[0].RetCode) {
            if (objRes[0].RetCode !== 0) {
                fnCallClientFailResult(objRes[0].ErrMsg);
                return false;
            }
        }

        if (objRes[0].result.ErrorCode !== 0) {
            fnCallClientFailResult(objRes[0].result.ErrorMsg);
            return false;
        }

        if (objRes[0].data.RecordCnt !== 1) {
            fnCallClientFailResult();
            return false;
        }

        var item = objRes[0].data.list[0];
        //Span
        $.each($("span"),
            function (index, input) {
                if (eval("item." + $(input).attr("id")) != null) {
                    $("#" + $(input).attr("id")).text(eval("item." + $(input).attr("id")));
                }
            });

        if ($("#ClientPost").text() !== "") {
            $("#ClientPost").text("[" + $("#ClientPost").text() + "]");
        }

        if ($("#ClientAcctName").text() !== "") {
            $("#ClientAcctName").text("(예금주 : " + $("#ClientAcctName").text() + ")");
        }
    } else {
        fnCallClientFailResult();
    }
}

function fnCallClientFailResult(msg) {
    msg = typeof msg === "undefined" || msg === null ? "" : ("(" + msg + ")");
    fnDefaultAlert("업체 정보를 불러오는데 실패했습니다." + msg, "error", "fnWindowClose()");
    return false;
}


/**********************************************************/
//확인내용등록
/**********************************************************/
//확인내용 등록
function fnInsNote() {
    if (!$("#HidCenterCode").val() || !$("#HidClientCode").val()) {
        return false;
    }

    if (!$("#NoteYMD").val()) {
        fnDefaultAlertFocus("확인일을 선택하세요.", "NoteYMD", "warning");
        return false;
    }

    if (!$("#Note").val()) {
        fnDefaultAlertFocus("확인내용을 입력하세요.", "Note", "warning");
        return false;
    }

    fnDefaultConfirm("확인내용을 등록하시겠습니까?", "fnInsNoteProc", "");
    return false;
}

function fnInsNoteProc() {
    var strHandlerURL = "/TMS/ClosingSaleDeposit/Proc/DepositMonthlyHandler.ashx";
    var strCallBackFunc = "fnInsNoteSuccResult";
    var strFailCallBackFunc = "fnInsNoteFailResult";
    var objParam = {
        CallType: "PayMisuNoteInsert",
        CenterCode: $("#HidCenterCode").val(),
        ClientCode: $("#HidClientCode").val(),
        NoteYMD: $("#NoteYMD").val(),
        Note: $("#Note").val()
    }
    UTILJS.Ajax.fnHandlerRequest(objParam, strHandlerURL, strCallBackFunc, true, strFailCallBackFunc, "", true);
}

function fnInsNoteSuccResult(objRes) {
    if (objRes) {
        if (objRes[0].RetCode === 0) {
            fnDefaultAlert("확인내용이 등록되었습니다.", "info");
            fnCallGridData(GridID);
            $("#BtnResetNote").click();
            return false;
        } else {
            fnInsNoteFailResult(objRes[0].ErrMsg);
            return false;
        }
    } else {
        fnInsNoteFailResult();
        return false;
    }
}

function fnInsNoteFailResult(msg) {
    msg = typeof msg === "undefined" || msg === null ? "" : ("(" + msg + ")");
    fnDefaultAlert("나중에 다시 시도해 주세요." + msg);
    return false;
}

//확인내용 수정
function fnUpdNote() {
    if (!$("#HidCenterCode").val() || !$("#HidClientCode").val()) {
        return false;
    }

    if (!$("#NoteSeqNo").val()) {
        fnDefaultAlert("선택된 확인내용이 없습니다.", "warning");
        return false;
    }

    if (!$("#NoteYMD").val()) {
        fnDefaultAlertFocus("확인일을 선택하세요.", "NoteYMD", "warning");
        return false;
    }

    if (!$("#Note").val()) {
        fnDefaultAlertFocus("확인내용을 입력하세요.", "Note", "warning");
        return false;
    }

    fnDefaultConfirm("확인내용을 수정하시겠습니까?", "fnUpdNoteProc", "");
    return false;
}

function fnUpdNoteProc() {
    var strHandlerURL = "/TMS/ClosingSaleDeposit/Proc/DepositMonthlyHandler.ashx";
    var strCallBackFunc = "fnUpdNoteSuccResult";
    var strFailCallBackFunc = "fnUpdNoteFailResult";
    var objParam = {
        CallType: "PayMisuNoteUpdate",
        NoteSeqNo: $("#NoteSeqNo").val(),
        CenterCode: $("#HidCenterCode").val(),
        ClientCode: $("#HidClientCode").val(),
        NoteYMD: $("#NoteYMD").val(),
        Note: $("#Note").val()
    }
    UTILJS.Ajax.fnHandlerRequest(objParam, strHandlerURL, strCallBackFunc, true, strFailCallBackFunc, "", true);
}

function fnUpdNoteSuccResult(objRes) {
    if (objRes) {
        if (objRes[0].RetCode === 0) {
            fnDefaultAlert("확인내용이 수정되었습니다.", "info");
            fnCallGridData(GridID);
            $("#BtnResetNote").click();
            return false;
        } else {
            fnUpdNoteFailResult(objRes[0].ErrMsg);
            return false;
        }
    } else {
        fnUpdNoteFailResult();
        return false;
    }
}

function fnUpdNoteFailResult(msg) {
    msg = typeof msg === "undefined" || msg === null ? "" : ("(" + msg + ")");
    fnDefaultAlert("나중에 다시 시도해 주세요." + msg);
    return false;
}

//확인내용 삭제
function fnDelNote() {
    if (!$("#HidCenterCode").val()) {
        fnDefaultAlert("업체 선택 후 입력하실 수 있습니다.", "warning");
        return false;
    }

    if (!$("#NoteSeqNo").val()) {
        fnDefaultAlert("선택된 확인내용이 없습니다.", "warning");
        return false;
    }

    fnDefaultConfirm("확인내용을 삭제하시겠습니까?", "fnDelNoteProc", "");
    return false;
}

function fnDelNoteProc() {
    var strHandlerURL = "/TMS/ClosingSaleDeposit/Proc/DepositMonthlyHandler.ashx";
    var strCallBackFunc = "fnDelNoteSuccResult";
    var strFailCallBackFunc = "fnDelNoteFailResult";
    var objParam = {
        CallType: "PayMisuNoteDelete",
        NoteSeqNo: $("#NoteSeqNo").val(),
        CenterCode: $("#HidCenterCode").val(),
        ClientCode: $("#HidClientCode").val()
    }
    UTILJS.Ajax.fnHandlerRequest(objParam, strHandlerURL, strCallBackFunc, true, strFailCallBackFunc, "", true);
}

function fnDelNoteSuccResult(objRes) {
    if (objRes) {
        if (objRes[0].RetCode === 0) {
            fnDefaultAlert("확인내용이 삭제되었습니다.", "info");
            fnCallGridData(GridID);
            $("#BtnResetNote").click();
            return false;
        } else {
            fnDelNoteFailResult(objRes[0].ErrMsg);
            return false;
        }
    } else {
        fnDelNoteFailResult();
        return false;
    }
}

function fnDelNoteFailResult(msg) {
    msg = typeof msg === "undefined" || msg === null ? "" : ("(" + msg + ")");
    fnDefaultAlert("나중에 다시 시도해 주세요." + msg);
    return false;
}

//확인내용 다시 입력
function fnResetNote() {
    $("#NoteSeqNo").val("");
    $("#NoteYMD").datepicker("setDate", new Date());
    $("#Note").val("");
    $("#BtnInsNote").show();
    $("#BtnUpdNote").hide();
    $("#BtnDelNote").hide();
}

//확인내용 데이터 세팅
function fnSetNote(objItem) {
    $("#NoteSeqNo").val(objItem.NoteSeqNo);
    $("#NoteYMD").val(fnGetStrDateFormat(objItem.NoteYMD, "-"));
    $("#Note").val(fnMoneyComma(objItem.Note));
    $("#BtnInsNote").hide();
    $("#BtnUpdNote").show();
    $("#BtnDelNote").show();
}
/**********************************************************/
