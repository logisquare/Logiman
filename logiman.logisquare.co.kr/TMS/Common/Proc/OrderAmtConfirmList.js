window.name = "OrderAmtConfirmGrid";
// 그리드
var GridID = "#OrderAmtConfirmGrid";
var GridSort = [];
...
function fnGridInit() {
    // 그리드 레이아웃 생성
    fnCreateGridLayout(GridID, "ReqSeqNo");
...
function fnCreateGridLayout(strGID, strRowIdField) {
    ...
    objGridProps.fixedColumnCount = 7; // 고정 칼럼 개수
    ...
    objGridProps.editableOnFixedCell = false; // 고정 칼럼, 행에 있는 셀도 수정 가능 여부(기본값:false)
    ...
    AUIGrid.bind(GridID, "columnStateChange", function (event) {
        fnSaveColumnLayoutAuto(GridID, "OrderAmtConfirmGrid");
        return;
    });
};
...
function fnGetDefaultColumnLayout() {
    var objColumnLayout = [
        ...
        {
            dataField: "BtnConfirmAmtRequest",
            headerText: "승인",
            editable: false,
            width: 100,
            renderer: {
                ...
                onClick: function(event) {
                    fnConfirmAmtRequest(event.item);
                }
            },
            viewstatus: true
        },
        {
            dataField: "BtnReturnAmtRequest",
            headerText: "반려",
            editable: false,
            width: 100,
            renderer: {
...
function fnCallGridData(strGID) {
    ...
    var strHandlerURL = "/TMS/Common/Proc/OrderAmtConfirmHandler.ashx";
    var strCallBackFunc = "fnGridSuccResult";

    var objParam = {
        CallType: "AmtRequestList",
...
//승인
function fnConfirmAmtRequest(objItem) {
    if (objItem.ReqSeqNo === "0" || typeof objItem.ReqSeqNo === "undefined") {
        fnDefaultAlert("등록된 수정요청 정보가 없습니다.", "warning");
        return false;
    }

    if (objItem.ReqStatus !== 1) {
        fnDefaultAlert("승인할 수 있는 수정요청 정보가 없습니다.", "warning");
        return false;
    }

    fnDefaultConfirm("승인 처리 하시겠습니까?", "fnConfirmAmtRequestProc", objItem);
    return false;
}

function fnConfirmAmtRequestProc(objItem) {
    var strHandlerURL = "/TMS/Common/Proc/OrderAmtConfirmHandler.ashx";
    var strCallBackFunc = "fnConfirmAmtRequestSuccResult";
    var strFailCallBackFunc = "fnConfirmAmtRequestFailResult";
    var objParam = {
        CallType: "AmtRequestStatusUpdate",
...
function fnConfirmAmtRequestSuccResult(objRes) {
    if (objRes) {
        if (objRes[0].RetCode === 0) {
            fnDefaultAlert("수정요청이 승인되었습니다.", "info");
            fnCallGridData(GridID);
            return false;
        } else {
            fnConfirmAmtRequestFailResult(objRes[0].ErrMsg);
            return false;
        }
    } else {
        fnConfirmAmtRequestFailResult();
        return false;
    }
}

function fnConfirmAmtRequestFailResult(msg) {
    var alertMsg = "수정요청 승인 처리에 실패했습니다. 나중에 다시 시도해 주시기 바랍니다.";
...
//반려
function fnReturnAmtRequest(objItem) {
    if (objItem.ReqSeqNo === "0" || typeof objItem.ReqSeqNo === "undefined") {
        fnDefaultAlert("등록된 수정요청 정보가 없습니다.", "warning");
        return false;
    }
    ...
    if (objItem.ReqStatus !== 1) {
        fnDefaultAlert("반려할 수 있는 수정요청 정보가 없습니다.", "warning");
        return false;
    }

    fnDefaultConfirm("반려 처리 하시겠습니까?", "fnReturnAmtRequestProc", objItem);
    return false;
}

function fnReturnAmtRequestProc(objItem) {
    var strHandlerURL = "/TMS/Common/Proc/OrderAmtConfirmHandler.ashx";
    var strCallBackFunc = "fnReturnAmtRequestSuccResult";
    var strFailCallBackFunc = "fnReturnAmtRequestFailResult";
    var objParam = {
        CallType: "AmtRequestStatusUpdate",
        CenterCode: objItem.CenterCode,
        OrderNo: objItem.OrderNo,
...
function fnReturnAmtRequestSuccResult(objRes) {
    if (objRes) {
        if (objRes[0].RetCode === 0) {
            fnDefaultAlert("수정요청이 반려되었습니다.", "info");
            fnCallGridData(GridID);
            return false;
        } else {
            fnReturnAmtRequestFailResult(objRes[0].ErrMsg);
            return false;
        }
    } else {
        fnReturnAmtRequestFailResult();
        return false;
    }
}

function fnReturnAmtRequestFailResult(msg) {
    var alertMsg = "수정요청 반려 처리에 실패했습니다. 나중에 다시 시도해 주시기 바랍니다.";
    if (typeof msg != "undefined" && msg !== "") {
        alertMsg += " (" + msg + ")";
    }
    fnDefaultAlert(alertMsg, "warning");
