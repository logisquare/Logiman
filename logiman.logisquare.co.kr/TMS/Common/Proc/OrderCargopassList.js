window.name = "OrderCargopassListGrid";
// 그리드
var GridID = "#OrderCargopassListGrid";
var GridSort = [];
...
function fnOpenOrderDetail(objItem) {
    ...
    window.open("/TMS/Common/OrderCargopassIns?CenterCode=" + strCenterCode + "&CargopassOrderNo=" + strCargopassOrderNo, "카고패스연동상세", "width=1350, height=800, scrollbars=Yes");
    return false;
}
...
function fnGetSessionSuccResult(objRes) {
    if (objRes) {
        if (objRes[0].RetCode === 0) {
            fnOpenCargopassExtListPost(objRes[0].ListURL, objRes[0].SessionKey, objRes[0].EncSession);
            return false;
        } else {
...
function fnOpenCargopassExtListPost(strUrl, strSessionKey, strEncSession) {
    var newForm = $("<form></form>");
    newForm.attr("method", "post");
    newForm.attr("action", strUrl);
    newForm.attr("target", "CargopassList");

    newForm.append($("<input/>", { type: "hidden", name: "SessionKey", value: strSessionKey }));
    newForm.append($("<input/>", { type: "hidden", name: "EncSession", value: strEncSession }));

    // 새 창에서 폼을 열기
    window.open("", "CargopassList", "width=1760, height=760, scrollbars=Yes");
    newForm.appendTo("body");
    newForm.submit();
    newForm.remove();
}