...
{
    public partial class OrderInoutPrintPeriod : PageBase
    {
        ...
        public static void CLIENT_CHARGE_DDLB(DropDownList DDLB, string lo_strCenterCode, string lo_strClientCode)
        {
            ...
            {
                SiteGlobal.WriteLog("OrderInoutPrintPeriod", "Exception",
        ...
        protected void GetPrintList(ServiceResult<ResOrderDispatchAdvanceList> lo_objResOrderDispatchAdvanceList)
        {
            ...
            try
            {
                ...
                if (lo_objResOrderDispatchList.data.RecordCnt > 0)
                {
                    ...
                    for (var i = 0; i < lo_strDistinctItemCode.Distinct().ToList().Count; i++)
                    {
                        if (!lo_strDistinctItemCode[i].Equals(""))
                        {
                            ...
                            {
                                if (lo_strDistinctItemCode[i].Equals(lo_objResOrderDispatchList.data.list[v].ItemCode))
                                {
                    ...
                    {
                        if (lo_strDistinctIndex.Count < 12)
                        {
                        ...
                        if (f.Equals(0))
                        {
                            ...
                            strTb += "</ul>";
                            strTb += "<h1 id=\"MailTitle\">운송내역서(기간)_(" + Utils.ConvertDateFormat(lo_objResOrderDispatchList.data.list[f].PickupYMD) + " ~ " + Utils.ConvertDateFormat(lo_objResOrderDispatchList.data.list[lo_objResOrderDispatchList.data.list.Count - 1].PickupYMD) + ")</h1>";
                        ...
                        for (var i = 0; i < lo_objResOrderDispatchList.data.list.Count; i++)
                        {
                            if (i.Equals(lo_strDistinctIndex[f]))
                            {
                                ...
                                strTb += "<td>" + lo_objResOrderDispatchList.data.list[i].CBM + "</td>";
                                strTb += "<td>" + lo_objResOrderDispatchList.data.list[i].PickupPlace + "</td>";
                                strTb += "<td>" + lo_objResOrderDispatchList.data.list[i].GetPlace + "</td>";
                                strTb += "<td>" + lo_objResOrderDispatchList.data.list[i].Nation + "</td>";
                                strTb += "<td>" + lo_objResOrderDispatchList.data.list[i].Hawb + "</td>";
                                strTb += "<td>" + (SupplyAmtTotal.Equals(0) ? "0" : SupplyAmtTotal.ToString("##,####")) + "</td>";
