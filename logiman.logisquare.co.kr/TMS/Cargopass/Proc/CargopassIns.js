...
function fnSetInitData() {
    ...
    if ($("#HidMode").val() === "Insert") {
        ...
        if (!$("#OrderNos").val() || !$("#DispatchType").val() || !$("#CenterCode").val()) {
            fnDefaultAlert("카고패스 정보 등록에 필요한 값이 없습니다.", "warning", "fnWindowClose()");
            return false;
        }
...
function fnSetOrderInfoSuccResult(objRes) {
    if (objRes) {
        ...
        if ($("#HidMode").val() === "Insert") {
            ...
            $.each($("#TblOrderList Tbody Tr"), function (index, item) {
                if ($(item).children("td:nth-child(4)").text() !== "") {
                    $("#ConsignorName").val($(item).children("td:nth-child(4)").text());
                    return false;
                }
            });
...
function fnOrderDetailSuccResult(objRes) {
    if (objRes) {
        ...
        if (objRes[0].data.RecordCnt !== 1) {
            fnAjaxFailResult("카고패스 정보를 불러오지 못했습니다.");
            return false;
        }
...
function fnInsCargopass() {
    ...
    if ($("#CargopassStatus").val() === "3") {
        fnDefaultAlert("배차 취소 후 등록할 수 있습니다.", "warning");
        return false;
    }
    ...
    strConfMsg = "카고패스 정보를 " + ($("#HidMode").val() === "Update" ? "수정" : "등록");
    strConfMsg += "하시겠습니까?";
...
function fnInsCargopassSuccResult(objRes) {
    ...
    if ($("#HidMode").val() === "Insert") {
        ...
        if ($("#InsCallback").val()) {
            parent.fnCargopassInsCallback(objRes[0].CargopassOrderNo);
        }
    }
...
function fnOrderReload() {
    document.location.replace("/TMS/Cargopass/CargopassIns?CenterCode=" + $("#CenterCode").val() + "&CargopassOrderNo=" + $("#CargopassOrderNo").val());
    return false;
}
    ...
    if (e.origin === $("#CargopassDomain").val()) {
        if (e.data.CallBackType) {
            ...
            if (e.data.CallBackType === "CargopassWindowResize") { //창 리사이즈
                ...
                fnCallCargopassDetail();
                return false;
            } else if (e.data.CallBackType === "CargopassCarConfirmCnlComplete") { //배차 확정취소 처리
                fnCallCargopassDetail();
                return false;
...
function fnCnlCallBack() {
    fnDefaultAlert("카고패스 등록이 취소되었습니다.", "warning", "fnWindowClose()");
    return false;
}
/************************************************************/